...
1. **Capture Sweep:** Run `scripts/capture/measure_transfer.py` (configured for `sweep` mode) to record the frequency response.
1. **Capture Linearity:** Run `scripts/capture/measure_transfer.py` (configured for `steady` mode with a triangle wave) to record the saturation curve.
1. **Analyze:** Use `notebooks/03_transfer_function.ipynb` to process the captured files.
