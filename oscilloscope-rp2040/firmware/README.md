...
1. **Zero-Copy Transmission:** Data is sent to the host using `memoryview()` objects. This allows the USB driver to read directly from the ADC buffer memory without burning CPU cycles on intermediate copies.
...
1. Copy `main.py` to the root of the device filesystem. I used [Thonny](https://thonny.org/) and would recommend it.
...
1. Zero jitter (derived strictly from the crystal oscillator).
1. Simultaneous processing (CPU free during capture).
